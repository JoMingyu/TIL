...
docker를 많이 쓰다 보면 수십GB씩 디스크를 차지해서 컨테이너가 disk space 문제로 제대로 안 뜨는 경우가 생길 수 있다. `docker system df` 명령어를 이용해 도커의 여러 요소들이 차지하는 disk space 정보를 볼 수 있다.
...
## Options
### verbose
`--verbose`, `-v` flag를 통해 verbose 모드를 켤 수 있다.
